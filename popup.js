...
(() => {
  ...
  chrome.runtime.sendMessage({ type: 'GET_STATE' }, (state) => {
    if (chrome.runtime.lastError) {
      console.error('[MoodLens popup] GET_STATE error:', chrome.runtime.lastError);
      return;
    }
  ...
  toggle.addEventListener('change', () => {
    ...
    updateDisabledState();
    updateStatus();
    chrome.runtime.sendMessage({ type: 'SET_ENABLED', enabled: currentEnabled });
  });
  ...
  const MOOD_GRADIENTS = {
    standard:          'linear-gradient(135deg, #64748b, #94a3b8)',
    cherry:            'linear-gradient(135deg, #f472b6, #fb923c)',
    honest:            'linear-gradient(135deg, #38bdf8, #818cf8)',
    'brutally-honest': 'linear-gradient(135deg, #ef4444, #f97316)',
    academic:          'linear-gradient(135deg, #6366f1, #3b82f6)',
    casual:            'linear-gradient(135deg, #34d399, #22d3ee)',
    poetic:            'linear-gradient(135deg, #a78bfa, #f0abfc)',
  };

  const MOOD_SHADOWS = {
    standard:          'rgba(148, 163, 184, 0.4)',
    cherry:            'rgba(244, 114, 182, 0.45)',
    honest:            'rgba(56, 189, 248, 0.4)',
    'brutally-honest': 'rgba(239, 68, 68, 0.5)',
    academic:          'rgba(99, 102, 241, 0.4)',
    casual:            'rgba(52, 211, 153, 0.4)',
    poetic:            'rgba(167, 139, 250, 0.45)',
  };
  ...
  function updateGlobe(moodData, isCustom) {
    ...
    globeSphere.style.setProperty('--custom-gradient', gradient || MOOD_GRADIENTS.standard);
    globeSphere.style.setProperty('--globe-shadow', shadow || MOOD_SHADOWS.standard);

    if (globeHue) {
      globeHue.style.setProperty('--custom-gradient', gradient || MOOD_GRADIENTS.standard);
    }
    ...
    if (inner) {
      ...
      setTimeout(() => { inner.style.transform = ''; }, 150);
    }
  }
  ...
  function renderCarousel() {
    ...
    addCard.innerHTML = `
      <span class="item-initial" style="font-size:18px;font-weight:700;">+</span>
  ...
  function createCarouselItem(mood, isCustom) {
    ...
    item.className = 'carousel-item' + (mood.id === currentMood ? ' selected' : '');
    item.dataset.mood = mood.id;

    // Use first letter as styled initial — no emoji
    item.innerHTML = `
      <span class="item-initial">${mood.name.charAt(0).toUpperCase()}</span>
    ...
    if (isCustom) {
      ...
      del.textContent = '✕';
      del.style.cssText = 'position:absolute;top:2px;right:2px;width:12px;height:12px;font-size:7px;line-height:1;border:none;background:rgba(0,0,0,0.3);color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;';
  ...
  function syncCarouselSelection() {
    ...
    const activeMood = BUILTIN_MOODS.find(m => m.id === currentMood) || customMoods.find(m => m.id === currentMood);
    if (activeMood) {
  ...
  function scrollToActiveItem() {
    ...
    let targetIdx = [...items].findIndex(el => el.dataset.mood === currentMood);
    if (targetIdx < 0) targetIdx = 0;

    const itemWidth = 59; // 54px item + 5px gap
    const viewportWidth = 274; // 380 - 36 padding - 22*2 arrows - 8*2 gaps
    const centerOffset = (viewportWidth - 54) / 2;
    carouselScrollOffset = -(targetIdx * itemWidth - centerOffset);

    const maxScroll = -(items.length * itemWidth - viewportWidth);
    if (carouselScrollOffset > 0) carouselScrollOffset = 0;
    if (carouselScrollOffset < maxScroll) carouselScrollOffset = maxScroll;

    carouselTrack.style.transform = `translateX(${carouselScrollOffset}px)`;
  }

  carouselPrev.addEventListener('click', () => {
    const items = [...carouselTrack.querySelectorAll('.carousel-item')].filter(el => el.dataset.mood);
    let activeIdx = items.findIndex(el => el.dataset.mood === currentMood);
    if (activeIdx > 0) {
      const prevMood = BUILTIN_MOODS.find(m => m.id === items[activeIdx - 1].dataset.mood) || customMoods.find(m => m.id === items[activeIdx - 1].dataset.mood);
      if (prevMood) selectMood(prevMood, customMoods.some(m => m.id === prevMood.id));
    }
  });
  ...
  carouselNext.addEventListener('click', () => {
    ...
    if (activeIdx >= 0 && activeIdx < items.length - 1) {
      const nextMood = BUILTIN_MOODS.find(m => m.id === items[activeIdx + 1].dataset.mood) || customMoods.find(m => m.id === items[activeIdx + 1].dataset.mood);
      if (nextMood) selectMood(nextMood, customMoods.some(m => m.id === nextMood.id));
    }
  });
  ...
  modalSave.addEventListener('click', () => {
    ...
    if (!desc) { customDesc.focus(); return; }
    ...
    const gradient = CUSTOM_GRADIENTS[customMoods.length % CUSTOM_GRADIENTS.length];

    const newMood = { id, name, desc, prompt, gradient };

    chrome.runtime.sendMessage({ type: 'SAVE_CUSTOM_MOOD', mood: newMood }, () => {
  ...
  restartBtn.addEventListener('click', () => {
    ...
    setTimeout(() => { restartBtn.style.transform = ''; }, 150);
  ...
  triggerBtn.addEventListener('click', () => {
    ...
    setTimeout(() => { triggerBtn.style.transform = ''; }, 150);
  ...
  function runFromInput() {
    ...
    const exists = savedCommands.some(c => c.text.toLowerCase() === text.toLowerCase());
    if (!exists) {
    ...
    setTimeout(() => { cmdRunBtn.style.transform = ''; }, 150);
  }
  ...
  function updateStatus() {
    ...
    const moodData = BUILTIN_MOODS.find(m => m.id === currentMood) || customMoods.find(m => m.id === currentMood);
  ...
  async function checkAiAvailability() {
    ...
    try {
      results = await chrome.scripting.executeScript({
        ...
        world: 'MAIN',
        func: () => {
          const lmGlobal = window.LanguageModel ?? self.LanguageModel ?? globalThis.LanguageModel;
    ...
    const aiPresent = report.hasLanguageModelGlobal || report.hasLanguageModel || report.hasAssistant;

    let detailStr = `LanguageModel: ${report.hasLanguageModelGlobal}`;
    if (!report.hasLanguageModelGlobal) detailStr += ` | ai: ${report.windowAi}`;
