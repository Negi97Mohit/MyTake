...
(() => {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (msg.type === "REQUEST_MODEL_UPDATE") {
      // content-main detected crashed model — ask background to trigger update
      try {
        chrome.runtime
  ...
  function getTargetedNodesForCommand(commandText, allNodes) {
    ...
    console.log(
      `${TAG} [Heuristic] Evaluating ${allNodes.length} nodes for command keyword targeting...`,
    );
    ...
    if (
      ...
      filtered = allNodes.filter((n) => {
        ...
        return /[$€£₹¥]|(\d+(\.\d+)?)\s*(usd|eur|gbp|inr|dollars|euros|rupees)?/i.test(
          text,
        );
      });
      console.log(
        `${TAG} [Heuristic] Price/Currency match: isolated ${filtered.length}/${allNodes.length} nodes.`,
      );
      return filtered;
    }
    ...
    if (
      ...
      console.log(
        `${TAG} [Heuristic] Numeric match: isolated ${filtered.length}/${allNodes.length} nodes.`,
      );
      return filtered;
    }
    ...
    if (/\b(email|phone|link|url|contact)\b/.test(cmd)) {
      ...
      console.log(
        `${TAG} [Heuristic] Contact/Link match: isolated ${filtered.length}/${allNodes.length} nodes.`,
      );
      return filtered;
    }
    ...
    if (
      ...
      console.log(
        `${TAG} [Heuristic] Proper Noun match: isolated ${filtered.length}/${allNodes.length} nodes.`,
      );
      return filtered;
    }
    ...
    if (/\b(summarize|shorten|simplify|explain|rewrite|rephrase)\b/.test(cmd)) {
      ...
      console.log(
        `${TAG} [Heuristic] Summarization/Simplification match: isolated ${filtered.length}/${allNodes.length} nodes.`,
      );
      return filtered;
    }
    ...
    console.log(
      `${TAG} [Heuristic] Default fallback: isolated ${filtered.length}/${allNodes.length} nodes.`,
    );
    return filtered;
  }
